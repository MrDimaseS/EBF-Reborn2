...
function VectorTarget:OrderFilter(event)
	...
	if event.entindex_target then
		target = EntIndexToHScript(event.entindex_target)
	end
	local ability = EntIndexToHScript(event.entindex_ability)
	
	if event.order_type == DOTA_UNIT_ORDER_DISASSEMBLE_ITEM then
		if string.find(ability:GetAbilityName(), "asura") then
			Timers:CreateTimer( function()
				for itemSlot = DOTA_ITEM_SLOT_1, DOTA_STASH_SLOT_6 do
					local asuraShard = unit:GetItemInSlot( itemSlot )
					if asuraShard ~= nil and asuraShard:GetAbilityName() == "item_orb_5" then
						asuraShard:SetCombineLocked( false )
						asuraShard:SetPurchaser( unit )
					end
				end
			end)
		end
