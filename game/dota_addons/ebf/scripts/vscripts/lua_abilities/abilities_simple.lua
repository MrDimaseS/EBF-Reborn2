...
function spawn_unit_arround( caster, unitname, radius, unit_number, playerID, core)
	...
	if not ( GameRules.holdOut._currentRound and GameRules.holdOut._currentRound.currentlyActive ) then return end
    for i = 0, unit_number-1 do
		local unit = CreateUnitByName( unitname ,caster:GetAbsOrigin() + RandomVector(RandomInt(radius,radius)), true, nil, nil, DOTA_TEAM_NEUTRALS ) 
...
function boss_evil_core_spawn(keys)
    ...
    Timers:CreateTimer(0.03,function()
        ...
        caster.shield_particle = ParticleManager:CreateParticle("pparticles/units/heroes/hero_doom_bringer/doom_bringer_doom_aura.vpcf", PATTACH_ABSORIGIN  , keys.caster)
        ...
        Timers:CreateTimer(0.06,function()
			...
			if GetMapName() == "epic_boss_fight_nightmare" then 
                sfx="_vh" 
                caster:SetMaxHealth(100000)
                caster:SetHealth(100000)
            end
        ...
        Timers:CreateTimer(0.25,function()
			...
			for _,unit in pairs ( Entities:FindAllByName( "npc_dota_creature")) do
				if unit:GetUnitName() ~= "npc_dota_boss36" and unit:GetTeamNumber() == DOTA_TEAM_NEUTRALS and unit:IsCreature() then
					weakness = false
				end
			end
			if weakness == true then 
				...
				if caster.weakness == true then
					caster.weakness = false
					caster.have_shield = true
					caster.shield_particle = ParticleManager:CreateParticle("particles/units/heroes/hero_doom_bringer/doom_bringer_doom_aura.vpcf", PATTACH_ABSORIGIN  , keys.caster)
						ParticleManager:SetParticleControl(caster.shield_particle, 0, origin)
						ParticleManager:SetParticleControl(caster.shield_particle, 1, size)
						ParticleManager:SetParticleControl(caster.shield_particle, 6, origin)
						ParticleManager:SetParticleControl(caster.shield_particle, 10, origin)
...
function boss_hell_guardian_death(keys)
    gold_rain(caster,10000,10000,25)
end
...
function boss_evil_core_take_damage(keys)
    ...
    if caster:GetHealth() > 50 and caster:IsAlive() then
        if caster.weakness == true then
        ...
        if not caster.dead or not caster:IsAlive() then
    ...
    for _,unit in pairs( units ) do
        ...
        Timers:CreateTimer(timer,function()
            ...
            if vDiff:Length2D() < Death_range then
                ...
                local deathDuration = 10
				ability:ApplyDataDrivenModifier( caster, unit, "target_warning", {duration = deathDuration} )
                Timers:CreateTimer(function()
					...
					if deathDuration > 0 and unit:HasModifier("target_warning") then
						deathDuration = deathDuration - 0.1
						return 0.1
					else
						unit.NoTombStone = false
					end
...
function projectile_death_orbs_hit( event )
    ...
    if target:GetHealth() <= target:GetMaxHealth()/4 then 
        ...
        elseif GetMapName() == "epic_boss_fight_hard"then
            ...
            target:SetHealth( math.ceil( target:GetHealth()*0.66 + 1 ) )
        end
    end
...
function hell_tempest_hit( event )
    ...
    if target.InWater ~= true then
        if target:GetUnitName()~="npc_dota_courier" and target:GetUnitName()~="npc_dota_flying_courier" then
            event.ability:DealDamage( event.caster, target, 100000, {damage_type = DAMAGE_TYPE_MAGICAL} )
        end
    end
end

function hell_tempest_boss_start( event )
	local caster = event.caster
	
	caster.tempestFX = ParticleManager:CreateParticle( "particles/units/heroes/hero_doom_bringer/doom_bringer_scorched_earth_buff.vpcf", PATTACH_POINT_FOLLOW, caster )
	caster.tempestFX2 = ParticleManager:CreateParticle( "particles/units/heroes/hero_doom_bringer/doom_scorched_earth.vpcf", PATTACH_POINT_FOLLOW, caster )
	ParticleManager:SetParticleControl( caster.tempestFX2, 1, Vector( 1000, 0, 0 ) )
end
...
function doom_raze( event )
    ...
    Timers:CreateTimer(0.25,function() 
            ...
            for i=1,8,1 do
                createAOEDamage(event,"particles/units/heroes/hero_nevermore/nevermore_shadowraze.vpcf",location,250,damage,DAMAGE_TYPE_MAGICAL,2,"soundevents/game_sounds_heroes/game_sounds_nevermore.vsndevts")
                location = location + rv* 275
            end
...
function hell_tempest_boss( keys )
    ...
    Timers:CreateTimer(delay, function()
		...
		ParticleManager:DestroyParticle(caster.tempestFX, true)
		ParticleManager:DestroyParticle(caster.tempestFX2, true)
        local projectileTable = {
            Ability = ability,
            ...
            iUnitTargetType = DOTA_UNIT_TARGET_ALL,
            bDeleteOnHit = false,
            vVelocity = caster:GetRightVector() * 600,
            vAcceleration = caster:GetForwardVector() * 200
        }
        ...
        Timers:CreateTimer(0.05, function()
            ...
            projectileTable.vVelocity = caster:GetRightVector() * 600
...
function doom_bringer_boss( event )
	...
	Timers:CreateTimer(0.5,function()
		if not caster or caster:IsNull() or not caster:IsAlive() or not target or target:IsNull() or not target:IsAlive() then
			ParticleManager:DestroyParticle(particle, false)
			return 
		end
		target:SetHealth( math.max( target:GetHealth()*hp_pct, 1 ) )
		duration = duration - 0.5
		if duration <= 0 or not target:IsAlive() then
			ParticleManager:DestroyParticle(particle, false)
			if target:IsAlive() and target:GetHealth() < target:GetMaxHealth() * 0.1 then
				target:ForceKill(true)
			end
		else
			return 0.5
...
function sacrifice(keys)
    ...
    for _,unit in ipairs ( Entities:FindAllByName( "npc_dota_hero*")) do
...
function spawn_unit( keys )
	...
	if GameRules.holdOut._currentRound then print( GameRules.holdOut._currentRound.currentlyActive, "round active" ) end
	if not ( GameRules.holdOut._currentRound and GameRules.holdOut._currentRound.currentlyActive ) then return end
    if keys.number_of_unit==nil then keys.number_of_unit=1 end
    for i = 0, keys.number_of_unit-1 do
        local entUnit = CreateUnitByName( unit ,caster:GetAbsOrigin() + RandomVector(RandomInt(400,400)), true, nil, nil, DOTA_TEAM_NEUTRALS )
    end
end
...
function rearm_refresh_cooldown( keys )
    ...
    local no_refresh_item = {["item_sheepstick_2"] = true,["item_ressurection_stone"] = true,["item_refresher"] = true,["item_cuirass_5"]= true}
    ...
    if caster:IsIllusion() == false and ability:IsCooldownReady() then
		...
		Timers:CreateTimer( 0.25, function()
			if caster and not caster:IsNull() and caster:IsAlive() then
				if cDur <= 0 then
					local steal_c = caster:FindModifierByName(modifierName_caster)
					if steal_c then
						steal_c:DecrementStackCount()
					end
					return
				end
				cDur = cDur - 0.25
		...
		Timers:CreateTimer( 0.25, function()
			if target and not target:IsNull() and target:IsAlive() then
				if tDur <= 0 then
					local steal_t = target:FindModifierByName(modifierName_target)
					if steal_t then
						steal_t:DecrementStackCount()
					end
					return
				end
				tDur = tDur - 0.25
