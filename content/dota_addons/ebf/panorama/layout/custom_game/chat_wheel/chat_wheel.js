...
function disableChatWheelLinesBasedOnMMR(playerMMR) {
...
function GetCurrentMMRForPlayer(playerId) {
    ...
    if (netTable != undefined && netTable.mmr != undefined) {
        ...
        return 0; // Return 0 instead of "N/A" for easier comparison
    }
}

function updateChatWheelLines() {
    const playerID = Game.GetLocalPlayerID();
    const mmr = GetCurrentMMRForPlayer(playerID);

    if (mmr !== 0) {
        $.Msg(`Updating chat wheel lines for player ${playerID} with MMR: ${mmr}`);
        disableChatWheelLinesBasedOnMMR(mmr);
    } else {
        $.Msg(`MMR data not found for player ${playerID}`);
        disableChatWheelLinesBasedOnMMR(0); // Disable all lines if MMR is not available
    }
}
...
(() => {
    ...
    CustomNetTables.SubscribeNetTableListener("mmr", (tableName, key, data) => {
        const playerID = Game.GetLocalPlayerID();
        if (key === playerID.toString() && data && data.mmr !== undefined) {
            $.Msg(`MMR updated for player ${playerID}: ${data.mmr}`);
            updateChatWheelLines();
        }
    });
})();