...
function meteor_on_spell_start(keys)
    ...
    Timers:CreateTimer({
        ...
        callback = function()
            ...
            if GetMapName() == "epic_boss_fight_impossible" or GetMapName() == "epic_boss_fight_challenger" or GetMapName() == "epic_boss_fight_nightmare" or GetMapName() == "epic_boss_fight_hard" then
...
function money_and_exp_gain(keys)
    if boss_meteor._round < 35 then
        if keys.caster:GetUnitName() == "npc_dota_boss32_trueform" or keys.caster:GetUnitName() == "npc_dota_boss32_trueform_h" or keys.caster:GetUnitName() == "npc_dota_boss32_trueform_vh" then
            ...
            for _,unit in pairs ( Entities:FindAllByName( "npc_dota_hero*")) do
                if unit:GetTeamNumber() == DOTA_TEAM_GOODGUYS then
                    if GetMapName() == "epic_boss_fight_impossible" or GetMapName() == "epic_boss_fight_challenger" or GetMapName() == "epic_boss_fight_nightmare" then
                        unit:AddExperience (18000,false,false)
                        if GameRules._NewGamePlus == true then
                            unit:AddExperience (180000,false,false)
                        end
                    else
                        unit:AddExperience (18000,false,false)
                        if GameRules._NewGamePlus == true then
            ...
            if GetMapName() == "epic_boss_fight_impossible" or GetMapName() == "epic_boss_fight_challenger" or GetMapName() == "epic_boss_fight_nightmare" then
                gold = 2500 * GoldMultiplier
            else
                gold = 3000 * GoldMultiplier
            end
            for i=0,10,1 do
