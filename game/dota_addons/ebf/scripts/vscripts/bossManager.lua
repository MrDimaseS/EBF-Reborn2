...
function bossManager:onBossSpawn(spawnedUnit)
	...
	if spawnedUnit:IsCreature() then
		...
		Timers:CreateTimer(function()
			...
			local HP_difficulty_multiplier = 1 + ((TeamCount()-1) * 0.75)^0.85
			local EHP_multiplier = HP_difficulty_multiplier/(1 + 0.8*(TeamCount()-1))
			local DMG_difficulty_multiplier = ( TeamCount() / 5 )^0.2 + ( TeamCount()-1 ) * 0.03
