...
function modifier_ebfr_lotus_orb_active:OnAbilityExecuted( params )
	if params.unit == self:GetCaster() and params.target == self:GetParent() then
		...
		if params.unit:IsSameTeam( params.target ) then
			for _, unit in ipairs( params.unit:FindFriendlyUnitsInRadius( params.target:GetAbsOrigin(), self.active_radius ) ) do
			...
			for _, unit in ipairs( params.unit:FindEnemyUnitsInRadius( params.target:GetAbsOrigin(), self.active_radius ) ) do
				if unit ~= params.target then
					newTarget = unit
					break
				end
