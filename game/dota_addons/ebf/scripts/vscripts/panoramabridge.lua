...
function panoramaBridge:Update_Health_Bar()
		...
		for _,unit in pairs ( Entities:FindAllByName( "npc_dota_creature")) do
			if unit:GetTeamNumber() == DOTA_TEAM_NEUTRALS then
				if unit.EHP_MULT and unit:GetMaxHealth()*unit.EHP_MULT > higgest_ennemy_hp and unit:IsAlive() then
					biggest_ennemy = unit
			...
			if biggest_ennemy ~= nil and not biggest_ennemy:IsNull() and biggest_ennemy:IsAlive() then
				...
				for nPlayerID = 0, DOTA_MAX_TEAM_PLAYERS-1 do
					if PlayerResource:GetTeam( nPlayerID ) == DOTA_TEAM_GOODGUYS then
						if PlayerResource:HasSelectedHero( nPlayerID ) then
							...
							if player and player.HB == true and player.Health_Bar_Open == false then
				...
				for nPlayerID = 0, DOTA_MAX_TEAM_PLAYERS-1 do
					if PlayerResource:GetTeam( nPlayerID ) == DOTA_TEAM_GOODGUYS then
						if PlayerResource:HasSelectedHero( nPlayerID ) then
							local player = PlayerResource:GetPlayer(nPlayerID)
							if player and player.HB == true and player.Health_Bar_Open == true then
