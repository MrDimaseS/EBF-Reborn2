...
function hoodwink_acorn_shot:FireAcorn(target, iBounces)
    ...
    -- Check for the talent and add the bonus bounces
    if caster:HasTalent("special_bonus_unique_hoodwink_acorn_shot_bounces") then
        local bonusBounces = caster:FindAbilityByName("special_bonus_unique_hoodwink_acorn_shot_bounces"):GetSpecialValueFor("value")
        bounces = bounces + bonusBounces
    else
    end
    ...
    self.projectileData[projIndex].totalDamage = 0
end
        ...
        if self.projectileData[projectile].bounces > 0 then
            for _, enemy in ipairs(caster:FindEnemyUnitsInRadius(position, self:GetCastRange())) do
                if enemy ~= target then
                    ...
                    self.projectileData[projIndex].totalDamage = data.totalDamage
