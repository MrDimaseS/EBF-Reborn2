witch_doctor_death_ward = class({})
witch_doctor_voodoo_switcheroo = class({})

function witch_doctor_death_ward:OnSpellStart()
	if IsServer() then
		local caster = self:GetCaster()
		local vPosition = self:GetCursorPosition()
		self.death_ward = CreateUnitByName("npc_dota_witch_doctor_death_ward", vPosition, true, caster, nil, caster:GetTeam())
		self.death_ward:SetControllableByPlayer(caster:GetPlayerID(), true)
		self.death_ward:SetOwner(caster)
		
		self.death_ward:AddNewModifier( caster, self, "modifier_witch_doctor_death_ward", {duration = self:GetChannelTime() } )
		
		EmitSoundOn("Hero_WitchDoctor.Death_WardBuild", self.death_ward)

		local damage = self:GetSpecialValueFor("damage")
		self.death_ward:SetAverageBaseDamage( damage, 0)
		
		local enemy = self.death_ward:FindRandomEnemyInRadius(self.death_ward:GetAbsOrigin(), self.death_ward:GetAttackRange() )
		self.death_ward:SetRangedProjectileName("particles/units/heroes/hero_witchdoctor/witchdoctor_ward_attack.vpcf")
		self.death_ward:SetForceAttackTarget( enemy )
	end
end

function witch_doctor_death_ward:OnChannelFinish()
	UTIL_Remove( self.death_ward )
end

function witch_doctor_death_ward:OnProjectileHit_ExtraData(target, vLocation, extraData)
...
function witch_doctor_death_ward:CreateBounceAttack(originalTarget, extraData)
    local caster = self:GetCaster()
...
LinkLuaModifier("modifier_death_ward_handling", "heroes/hero_witch_doctor/witch_doctor_death_ward", LUA_MODIFIER_MOTION_NONE)
